...
interface EditorStore {
  ...
  openFile: (projectId: Id<"projects">, fileId: Id<"files">, options: { pinned: boolean }) => void; // pinned show if its for preview use or not
...
export const useEditorStore = create<EditorStore>()((set, get) => ({
  ...
  openFile: (projectId, fileId, { pinned }) => {
    ...
    //Case 1 : Opening as preview - replace existing preview or add new preview window
    if (!isOpen && !pinned) {
    ...
    //Case 2 : Opening as a new pinned tab
    if (!isOpen && pinned) {
      tabs.set(projectId, {
        ...state,
    ...
    //Case 3 : making a preview tab as pinned tab
